...
class PageSetupCard: public Page {
    ...
    public:
        ...
        void OnChange(bool force_update) override
        {
            ...
            if (config->HasParameterChanged(CARD_SDCARD_STATE) || config->HasParameterChanged(CARD_SDCARD) || force_update) {
                // // logic for the icons
                ...
                // update card-info
                lv_obj_t* sdInfo[2] = {objects.setup_card_sd1info, objects.setup_card_sd2info};
                
                for (uint i = 0; i < 2; i++)
                {  
                    String sdtext = config->GetParameter(CARD_SDCARD_STATE)->GetFormatedValue(i);
                    lv_label_set_text(sdInfo[i], sdtext.c_str());
                }
